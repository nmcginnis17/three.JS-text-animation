// Allows animation to run upon window load
window.onload = init;
...
function init() {
  ...
  // Sets up scene where text animation will take place
  root.renderer.setClearColor(0x3AAFA9);
  root.renderer.setPixelRatio(window.devicePixelRatio || 1);
  root.camera.position.set(0, 0, 400);

  // Creates text animation variable and assigns function createTextAnimation()
  // Identifies the exact position based on the y-axis where the animation will play at.
  var textAnimation = createTextAnimation();
  textAnimation.position.y = -10;
  root.scene.add(textAnimation);

  // Set time line aspect of animation
  var t1 = new TimelineMax({
    repeat: -1,
    repeatDelay: 0.25,
    yoyo: true
  });
  t1.fromTo(textAnimation, 4, //4
...
function createTextAnimation() {
  var geometry = generateTextGeometry('Now you can read me...', {
    size: 14,
    height: 0,
    font: 'droid sans',
    weight: 'bold',
